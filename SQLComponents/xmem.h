...
{
public:
    XMem(char *naam,int trace);
   ~XMem();
    // Interface - public methods
    void*  x_malloc(unsigned int blocksize,                       char* filename,int lineno);
    void*  x_calloc(unsigned int elements,unsigned int blocksize, char* filename,int lineno);
    void*  x_realloc(void *,unsigned int blocksize,               char* filename,int lineno);
    void   x_free(void *);
    void   x_chkfree(void);
    void*  x_strdup(void *s,char* filename,int lineno);
private:
    // Private methods
    char*  sto_ptr(char *p,unsigned int block,char* filename,int lineno);
    void   del_ptr(char *p);
    void   dump_block(ALLOCATION* p_alloc);
    void   trace(char* p_format,...);
    // Data members
    int 	 m_memtrace;
    int 	 m_hashsize;
    int 	 m_bucketsize;
