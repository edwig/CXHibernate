...
char *
XMem::sto_ptr(char *p,unsigned int b,char* filename,int lineno)
{
	register BUCKET *bp,*bq;
	register int    bno;
	if(!m_ptrhash)
	{
	...
	bno = (int)((unsigned long) p % m_hashsize);
...
void *
XMem::x_malloc(unsigned int b,char* filename,int lineno)
{
	register char *mptr;
	if(m_memtrace) 
	{
...
void *
XMem::x_calloc(unsigned int elements,unsigned int blocksize,char* filename,int lineno)
{
...
XMem::x_realloc(void *p,size_t s,char* filename,int lineno)
{
	...
	if(m_memtrace)
	{
		bno = (short)((unsigned long)p % m_hashsize);
